...
void handleI2CError2(i2c_inst_t* &i2c_port) {
    if (++i2c_reinit_attempts >= max_i2c_reinit_attempts ) {
        initI2C2(i2c_port, true); // Force recovery on the nth attempt
        i2c_reinit_attempts = 0; // Reset attempts after recovery
    } else {
        initI2C2(i2c_port, false); // Regular reinitialization without forcing recovery
    }
}
...
void initI2C2(i2c_inst_t* &i2c_port, bool force_recovery) {
    ...
    if (force_recovery) {
        i2cBusRecovery2(CONFIG_2::I2C_SDA_PIN, CONFIG_2::I2C_SCL_PIN);
    }
    // de-initialise I2C pins in case
    gpio_disable_pulls(CONFIG_2::I2C_SDA_PIN);
    gpio_set_function(CONFIG_2::I2C_SDA_PIN, GPIO_FUNC_NULL);
    gpio_disable_pulls(CONFIG_2::I2C_SCL_PIN);
    gpio_set_function(CONFIG_2::I2C_SCL_PIN, GPIO_FUNC_NULL);

    // initialise I2C with baudrate
    i2c_init(i2c_port, CONFIG_2::I2C_BAUD_RATE);

    gpio_set_function(CONFIG_2::I2C_SDA_PIN, GPIO_FUNC_I2C);
    gpio_set_function(CONFIG_2::I2C_SCL_PIN, GPIO_FUNC_I2C);
    gpio_pull_up(CONFIG_2::I2C_SDA_PIN);
    gpio_pull_up(CONFIG_2::I2C_SCL_PIN);
}
