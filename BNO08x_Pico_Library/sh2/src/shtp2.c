...
#define SH2_MAX_APPS_2 (5)
#define SHTP_APP_NAME_LEN_2 (32)
#define SH2_MAX_CHANS_2 (8)
#define SHTP_CHAN_NAME_LEN_2 (32)

// Defined Globally Unique Identifiers
#define GUID_SHTP_2 (0)

// Command Channel commands and parameters
#define SHTP_CHAN_COMMAND_2 (0)
#define CMD_ADVERTISE_2 (0)
#define CMD_ADVERTISE_SHTP_2 (0)
#define CMD_ADVERTISE_ALL_2 (1)
#define RESP_ADVERTISE_2 (0)

#define SHTP_HDR_LEN_2 (4)

#define TAG_SHTP_VERSION_2 0x80

typedef struct shtp_App_s_2 {
    uint32_t guid;
    char appName[SHTP_APP_NAME_LEN_2];
} shtp_App_t_2;

typedef struct shtp_AppListener_s_2 {
    uint16_t guid;
    shtp_AdvertCallback_t_2 *callback;
    void *cookie;
} shtp_AppListener_t_2;

typedef struct shtp_ChanListener_s_2 {
    uint16_t guid;
    char chanName[SHTP_CHAN_NAME_LEN_2];
    shtp_Callback_t_2 *callback;
    void *cookie;
} shtp_ChanListener_t_2;

typedef struct shtp_Channel_s_2 {
    uint8_t nextOutSeq;
    uint8_t nextInSeq;
...
typedef struct shtp_s_2 {
    ...
    // Transmit support
    uint8_t outTransfer[SH2_HAL_MAX_TRANSFER_OUT_2];

    // Receive support
    uint16_t inMaxTransfer;
    uint16_t inRemaining;
    uint8_t  inChan;
    uint8_t  inPayload[SH2_HAL_MAX_PAYLOAD_IN_2];
    uint16_t inCursor;
    uint32_t inTimestamp;
    uint8_t inTransfer[SH2_HAL_MAX_TRANSFER_IN_2];
    
    // What stage of advertisement processing are we in.
    advert_phase_t_2 advertPhase;
    
    // Applications
    shtp_App_t_2 app[SH2_MAX_APPS_2];
    uint8_t    nextApp;
    
    // Advert registrations
    uint8_t nextAppListener;
    shtp_AppListener_t_2 appListener[SH2_MAX_APPS_2];

    // SHTP Channels
    shtp_Channel_t_2      chan[SH2_MAX_CHANS_2];

    // Channel listeners
    shtp_ChanListener_t_2 chanListener[SH2_MAX_CHANS_2];
...
static const uint8_t advertise_2[] = {
    CMD_ADVERTISE_2,
    CMD_ADVERTISE_ALL_2
};

#define MAX_INSTANCES_2 (1)
static shtp_t_2 instances_2[MAX_INSTANCES_2];

static bool shtp_initialized_2 = false;

// ------------------------------------------------------------------------
// Private functions

static void shtp_init_2(void)
{
    // clear instance memory.
    // In particular, this clears the pHal pointers which are used
    // to determine if an instance is open and in-use.
    memset(instances_2, 0, sizeof(instances_2));

    // Set the initialized flag so this doesn't happen again.
    shtp_initialized_2 = true;
}

static shtp_t_2 *getInstance_2(void)
{
    for (int n = 0; n < MAX_INSTANCES_2; n++) {
        if (instances_2[n].pHal == 0) {
            // This instance is free
            return &instances_2[n];
        }
    }
...
static void addAdvertListener_2(shtp_t_2 *pShtp, uint16_t guid,
                              shtp_AdvertCallback_t_2 *callback, void * cookie)
{
    shtp_AppListener_t_2 *pAppListener = 0;

    // Bail out if no space for more apps
    if (pShtp->nextAppListener >= SH2_MAX_APPS_2) return;
...
static void updateCallbacks_2(shtp_t_2 *pShtp)
{
    ...
    for (int chanNo = 0; chanNo < SH2_MAX_CHANS_2; chanNo++) {
        ...
        for (int listenerNo = 0; listenerNo < SH2_MAX_CHANS_2; listenerNo++)
        {
...
static int addChanListener_2(shtp_t_2 *pShtp,
                           uint16_t guid, const char * chanName,
                           shtp_Callback_t_2 *callback, void *cookie)
{
    shtp_ChanListener_t_2 *pListener = 0;

    // Bail out if there are too many listeners registered
    if (pShtp->nextChanListener >= SH2_MAX_CHANS_2) return SH2_ERR_2;
    ...
    updateCallbacks_2(pShtp);

    return SH2_OK_2;
}

static inline uint16_t min_u16_2(uint16_t a, uint16_t b)
{
    if (a < b) {
        return a;
    }
...
static int txProcess_2(shtp_t_2 *pShtp, uint8_t chan, const uint8_t* pData, uint32_t len)
{
    int status = SH2_OK_2;
    ...
    while (remaining > 0) {
        ...
        transferLen = min_u16_2(remaining, pShtp->outMaxTransfer-SHTP_HDR_LEN_2);
        
        // Length field will be transferLen + SHTP_HDR_LEN
        lenField = transferLen + SHTP_HDR_LEN_2;
        ...
        memcpy(pShtp->outTransfer+SHTP_HDR_LEN_2, pData+cursor, transferLen);
...
static void shtpAdvertHdlr_2(void *cookie, uint8_t tag, uint8_t len, uint8_t *val)
{
    shtp_t_2 *pShtp = (shtp_t_2 *)cookie;

    switch (tag) {
        case TAG_SHTP_VERSION_2:
...
static void addApp_2(shtp_t_2 *pShtp, uint32_t guid)
{
    shtp_App_t_2 *pApp = 0;
    ...
    // Bail out if no space for more apps
    if (pShtp->nextApp >= SH2_MAX_APPS_2) return;
...
static void setAppName_2(shtp_t_2 *pShtp, uint32_t guid, const char * appName)
{
    shtp_App_t_2 *pApp = 0;
...
static void addChannel_2(shtp_t_2 *pShtp, uint8_t chanNo, uint32_t guid, const char * chanName, bool wake)
{
    if (chanNo >= SH2_MAX_CHANS_2) return;

    shtp_Channel_t_2 * pChan = &pShtp->chan[chanNo];
...
static void callAdvertHandler_2(shtp_t_2 *pShtp, uint32_t guid,
                              uint8_t tag, uint8_t len, uint8_t *val)
{
    // Find listener for this app
    for (int n = 0; n < SH2_MAX_APPS_2; n++)
    {
...
static void processAdvertisement_2(shtp_t_2 *pShtp, uint8_t *payload, uint16_t payloadLen)
{
    uint16_t x;
    uint8_t tag;
    uint8_t len;
    ...
    pShtp->advertPhase = ADVERT_IDLE_2;
    ...
    while (cursor < payloadLen) {
        ...
        switch (tag) {
            ...
            case TAG_GUID_2:
                ...
                callAdvertHandler_2(pShtp, guid, TAG_NULL_2, 0, 0);
                ...
                strcpy(chanName, "");
                break;
            case TAG_MAX_CARGO_PLUS_HEADER_WRITE_2:
                x = readu16(val) - SHTP_HDR_LEN_2;
            
                if (x < SH2_HAL_MAX_PAYLOAD_OUT_2) {
                    pShtp->outMaxPayload = x;
                }
                break;
            case TAG_MAX_CARGO_PLUS_HEADER_READ_2:
                x = readu16(val) - SHTP_HDR_LEN_2;
                // No need to store this!
                break;
            case TAG_MAX_TRANSFER_WRITE_2:
                x = readu16(val) - SHTP_HDR_LEN_2;
                if (x < SH2_HAL_MAX_TRANSFER_OUT_2) {
                    pShtp->outMaxTransfer = x;
                } else {
                    pShtp->outMaxTransfer = SH2_HAL_MAX_TRANSFER_OUT_2;
                }
                break;
            case TAG_MAX_TRANSFER_READ_2:
                x = readu16(val) - SHTP_HDR_LEN_2;
                if (x < SH2_HAL_MAX_TRANSFER_IN_2) {
                    pShtp->inMaxTransfer = x;
                }
                break;
            case TAG_NORMAL_CHANNEL_2:
                chanNo = readu8(val);
                wake = false;
                break;
            case TAG_WAKE_CHANNEL_2:
                chanNo = readu8(val);
                wake = true;
                break;
            case TAG_APP_NAME_2:
                strcpy(appName, (const char *)val);
                setAppName_2(pShtp, guid, appName);
            
                break;
            case TAG_CHANNEL_NAME_2:
                strcpy(chanName, (const char *)val);
                addChannel_2(pShtp, chanNo, guid, (const char *)val, wake);

                // Store channel metadata
                if (chanNo < SH2_MAX_CHANS_2) {
                    pShtp->chan[chanNo].guid = guid;
                    strcpy(pShtp->chan[chanNo].chanName, chanName);
                    pShtp->chan[chanNo].wake = wake;
                }
                break;
            case TAG_ADV_COUNT_2:
                // Not yet supported.
                break;
            default:
        ...
        callAdvertHandler_2(pShtp, guid, tag, len, val);
    }
...
static void shtpCmdListener_2(void *cookie, uint8_t *payload, uint16_t len, uint32_t timestamp)
{
    shtp_t_2 *pShtp = (shtp_t_2 *)cookie;
    ...
    switch (response) {
        case RESP_ADVERTISE_2:
            processAdvertisement_2(pShtp, payload, len);
            break;
        default:
            // unknown response
            break;
...
static void rxAssemble_2(shtp_t_2 *pShtp, uint8_t *in, uint16_t len, uint32_t t_us)
{
    uint16_t payloadLen;
    bool continuation;
    uint8_t chan = 0;
    ...
    if (payloadLen < SHTP_HDR_LEN_2) {
      pShtp->shortFragments++;

      if (pShtp->eventCallback) {
          pShtp->eventCallback(pShtp->eventCookie, SHTP_SHORT_FRAGMENT_2);
      }
      return;
    }
        
    if ((chan >= SH2_MAX_CHANS_2) ||
        (chan >= pShtp->nextChanListener)) {
        // Invalid channel id.
        pShtp->badRxChan++;

        if (pShtp->eventCallback) {
            pShtp->eventCallback(pShtp->eventCookie, SHTP_BAD_RX_CHAN_2);
        }
        return;
    }
    ...
    if (pShtp->inRemaining == 0) {
        if (payloadLen > sizeof(pShtp->inPayload)) {
            ...
            if (pShtp->eventCallback) {
                pShtp->eventCallback(pShtp->eventCookie, SHTP_TOO_LARGE_PAYLOADS_2);
            }
            return;
        }
    ...
    memcpy(pShtp->inPayload + pShtp->inCursor, in+SHTP_HDR_LEN_2, len-SHTP_HDR_LEN_2);
    pShtp->inCursor += len-SHTP_HDR_LEN_2;
...
void *shtp_open_2(sh2_Hal_t_2 *pHal)
{
    ...
    memset(pShtp, 0, sizeof(shtp_t_2));
    ...
    pShtp->outMaxPayload = SH2_HAL_MAX_PAYLOAD_OUT_2;
    pShtp->outMaxTransfer = SH2_HAL_MAX_TRANSFER_OUT_2;

    // Establish SHTP App and command channel a priori
    addApp_2(pShtp, GUID_SHTP_2);
    addChannel_2(pShtp, 0, GUID_SHTP_2, "command", false);
    
    // Register SHTP advert listener and command channel listener
    shtp_listenAdvert_2(pShtp, GUID_SHTP_2, shtpAdvertHdlr_2, pShtp);
    shtp_listenChan_2(pShtp, GUID_SHTP_2, "command", shtpCmdListener_2, pShtp);
    ...
    pShtp->advertPhase = ADVERT_REQUESTED_2;
...
void shtp_close_2(void *pInstance)
{
    shtp_t_2 *pShtp = (shtp_t_2 *)pInstance;
...
int shtp_listenChan_2(void *pInstance,
                    ...
                    shtp_Callback_t_2 *callback, void * cookie)
{
    ...
    if ((chan == 0) || (strlen(chan) == 0)) return SH2_ERR_BAD_PARAM_2;

    return addChanListener_2(pShtp, guid, chan, callback, cookie);
}

// Register a listener for SHTP advertisements 
int shtp_listenAdvert_2(void *pInstance,
                      uint16_t guid,
                      shtp_AdvertCallback_t_2 *advertCallback, void * cookie)
{
    ...
    addAdvertListener_2(pShtp, guid, advertCallback, cookie);

    // Arrange for a new set of advertisements, for this listener
    if (pShtp->advertPhase == ADVERT_IDLE_2) {
        pShtp->advertPhase = ADVERT_NEEDED_2;
    }
...
uint8_t shtp_chanNo_2(void *pInstance,
                    const char * appName, const char * chanName)
{
    ...
    for (chan = 0; chan < SH2_MAX_CHANS_2; chan++) {
...
int shtp_send_2(void *pInstance,
              uint8_t channel, const uint8_t *payload, uint16_t len)
{
    ...
    if (channel >= SH2_MAX_CHANS_2) {
        pShtp->badTxChan++;
        return SH2_ERR_BAD_PARAM_2;
    }
    
    ret = txProcess_2(pShtp, channel, payload, len);

    return ret;
}

// Check for received data and process it.
void shtp_service_2(void *pInstance)
{
    shtp_t_2 *pShtp = (shtp_t_2 *)pInstance;
    uint32_t t_us = 0;

    if (pShtp->advertPhase == ADVERT_NEEDED_2) {
        pShtp->advertPhase = ADVERT_REQUESTED_2;  // do this before send, to avoid recursion.
        int status = shtp_send_2(pShtp, SHTP_CHAN_COMMAND_2, advertise_2, sizeof(advertise_2));
        if (status != SH2_OK_2) {
    ...
    if (len) {
        rxAssemble_2(pShtp, pShtp->inTransfer, len, t_us);
    }
}
